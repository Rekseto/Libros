...
export default function* booksSaga() {
  yield takeLatest("USERS_FETCH_REQUEST", function* (action) {
    try {
      ...
      const data = yield call(fetch, usersEndpoint + action.payload.page + `?${queryString.stringify({ username: action.payload.username })}`, {
        method: "GET",
        headers
      });
      ...
      yield put(usersActions.usersFetchFailed({
